...
### Q9: GPS 정확도 필터링이 왜 필요한가요?

**A**: GPS는 환경에 따라 정확도가 크게 달라지며, 다단계 필터링 체인을 통해 정밀한 위치 데이터를 확보합니다.

**필터 체인**: 정확도 필터 → 칼만 필터 → 속도 필터
...
### Q8: 최소 거리 필터링이 왜 필요한가요?

**A**: GPS 노이즈를 제거하고 활동 상태에 따른 적절한 위치 샘플링을 위함입니다.

**활동 상태별 최소 거리**:
- **걷기**: 4m 이동 시 위치 추가
- **달리기**: 5m 이동 시 위치 추가
- **정지**: 10m 이동 시 위치 추가 (거의 이동 안 함)
...
```kotlin
// 활동 상태에 따른 최소 거리 적용
val minDistance = when (currentActivity) {
    ActivityType.WALKING -> MIN_DISTANCE_WALKING    // 4m
    ActivityType.RUNNING -> MIN_DISTANCE_RUNNING    // 5m
    ActivityType.STILL -> MIN_DISTANCE_STILL        // 10m
    else -> MIN_DISTANCE_DEFAULT                    // 4m
}
...
#### 2. 위치 업데이트 대기 시간 (최대 1-16초)
- 활동 상태 + 배터리 상태에 따라 위치 업데이트 간격이 다름
- 기본: 달리기 1초, 걷기 2초, 정지 8초, 차량 5초
- 배터리 부족 시: 간격 1.5-2배 증가 (배터리 50% 이상: 정상, 20-50%: 1.5배, 20% 미만: 2배)
...
**이유**:
- **달리기 (1초)**: 매우 빠르게 움직이므로 가장 자주 업데이트
- **걷기 (2초)**: 적당한 속도이므로 중간 간격
- **정지 (8초)**: 움직이지 않으므로 드물게 업데이트
- **차량 (5초)**: 빠른 이동이나 GPS 신호 변동 고려
...
- [감정 기반 산책 가이드](../README.md#-감정-기반-산책-추적)에서 앱 사용법 확인