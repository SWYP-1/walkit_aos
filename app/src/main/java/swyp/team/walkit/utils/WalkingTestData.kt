...
object WalkingTestData {
    ...
    private fun generateActivityStats(
        duration: Long,
        totalDistance: Float,
    ): List<swyp.team.walkit.data.model.ActivityStats> {
        val stats = mutableListOf<swyp.team.walkit.data.model.ActivityStats>()

        // WALKING이 대부분 (80%)
        val walkingDuration = (duration * 0.8).toLong()
        val walkingDistance = totalDistance * 0.8f
        stats.add(
            swyp.team.walkit.data.model.ActivityStats(
